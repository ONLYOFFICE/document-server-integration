...
namespace OnlineEditorsExample
{
    public class TrackManager
    {
        ...
        public static int processSave(Dictionary<string, object> fileData, string fileName, string userAddress)
        {
            ...
            var curExt = Path.GetExtension(fileName);  // get current file extension
            var downloadExt = Path.GetExtension(downloadUri) ?? "";  // get the extension of the downloaded file
        ...
        public static int processForceSave(Dictionary<string, object> fileData, string fileName, string userAddress)
        {
            ...
            if (!curExt.Equals(downloadExt))
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(newFileUri))
                    {
                        newFileName = _Default.GetCorrectName(Path.GetFileNameWithoutExtension(fileName) + downloadExt, userAddress);  // get the correct file name if it already exists
                    }
                    else
                    {
                        downloadUri = newFileUri;
            ...
            if (isSubmitForm)
            {
                ...
                DocEditor.CreateMeta(newFileName, user, "Filling Form", userAddress);  // create meta data for the forcesaved file
            }
