...
var anonymous_user = models.User{Id: "uid-4", Username: "Anonymous", Description: []string{
...
func (um DefaultUserManager) GetUserById(user_id string) (models.User, error) {
	users := um.GetUsers()

	um.logger.Debugf("Trying to get a user by id=%s", user_id)
	for _, user := range users {
		if user.Id == user_id {
			return user, nil
		}
	}
	return anonymous_user, nil
}
